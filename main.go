...
const socketPath = "/run/docker/plugins/deny-userns-mode-host.sock"
...
// denyUsernsModeHost denies all requests and responses that have
// { "HostConfig": { "UsernsMode": "host" } } set in the request body.
func denyUsernsModeHost(w http.ResponseWriter, r *http.Request) {
	defer r.Body.Close()
	var req authzReq
...
func main() {
	log.Info("deny-userns-mode-host Docker authz plugin starting.")
	socket := listenUnix()
	...
	http.HandleFunc("/AuthZPlugin.AuthZReq", denyUsernsModeHost)
	http.HandleFunc("/AuthZPlugin.AuthZRes", denyUsernsModeHost)
